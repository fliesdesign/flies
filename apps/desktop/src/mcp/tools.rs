...
Pages: a Flies file holds one or more pages, and each page is a separate canvas with its own artwork. Every tool reads and writes the active page only: get_basic_info and list_pages report the pages and activePageId, set_page switches which one is active, create_page adds one and makes it active, and delete_page removes a page with all of its layers. A file always keeps at least one page. Use separate pages for genuinely separate surfaces (marketing site, app, explorations); use separate artboards inside one page for screens that belong together. An artboard is a frame on a page, not a page. Selection is scoped to the active page; set_selection rejects layers on other pages, so call set_page first.
