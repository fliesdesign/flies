...
pub fn catalog() -> Vec<Value> {
    ...
    vec![
        ...
        tool("set_theme", "Create or update theme tokens by stable ID. Existing IDs merge by default; replace:true replaces the full theme. deleteTokenIds removes tokens, preserving current values on linked layers. Bound layers update atomically with undo and save. Types: color (6/8-digit hex), fontFamily (family name), spacing/radius/fontSize (pixels). Every token is available as CSS var(--id) in write_html and preview_html; use apply_tokens to retain live links on native layers.", json!({"tokens":{"type":"array","maxItems":500,"items":{"type":"object","additionalProperties":false,"required":["id","name","type","value"],"properties":{"id":{"type":"string","pattern":"^[a-z][a-z0-9-]{0,79}$"},"name":{"type":"string","minLength":1,"maxLength":120},"type":{"enum":["color","fontFamily","spacing","radius","fontSize"]},"value":{"type":["string","number"]}}}},"replace":{"type":"boolean"},"deleteTokenIds":{"type":"array","items":{"type":"string"}}}), &["tokens"], false),
        tool("apply_tokens", "Bind selected node properties to theme token IDs. Changes to a token update all bound layers. null detaches a binding while keeping the current literal value. Properties must match token types and node kinds; fill maps to text color or pen stroke when appropriate. Literal property edits detach their corresponding token.", json!({"nodeIds":ids,"bindings":{"type":"object","additionalProperties":false,"properties":{"fill":{"type":["string","null"]},"color":{"type":["string","null"]},"stroke":{"type":["string","null"]},"borderColor":{"type":["string","null"]},"fontFamily":{"type":["string","null"]},"fontSize":{"type":["string","null"]},"cornerRadius":{"type":["string","null"]},"layoutGap":{"type":["string","null"]},"layoutPadding":{"type":["string","null"]},"letterSpacing":{"type":["string","null"]},"borderWidth":{"type":["string","null"]},"strokeWidth":{"type":["string","null"]}}}}), &["nodeIds","bindings"], false),
...
Theme tokens: call set_theme({tokens:[{id:"brand",name:"Brand",type:"color",value:"#6366f1"},{id:"body-font",name:"Body",type:"fontFamily",value:"Inter"},{id:"space-md",name:"Medium",type:"spacing",value:24}]}) once per file. Theme tokens persist and have undo history. Use CSS var(--brand), var(--body-font), var(--space-md) in imports/previews. To keep native layers linked, apply_tokens({nodeIds:[NODE_ID],bindings:{fill:"brand",fontFamily:"body-font"}}) on applicable nodes. get_theme reports tokens and usage; updates propagate to bound layers. Null bindings detach; deleting a token leaves the current rendered value. The left sidebar's Theme tab edits the same tokens; Design pickers can select them.
