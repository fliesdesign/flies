...
Rendering: the optional Use Firefly renderer project-menu setting changes artwork rendering only. All tools, imports, selection, undo, and autosave share the same live document in either renderer mode. get_screenshot continues to use the shared HTML export renderer.
