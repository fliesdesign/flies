...
pub fn catalog() -> Vec<Value> {
    ...
    let mut tools = vec![
        ...
        tool("get_node_info", "Get a node's saved properties, transformed worldBounds and direct child IDs.", json!({"nodeId":string}), &["nodeId"], true),
        ...
        tool("update_node", "Patch a node using native Flies properties. Bounds use document layout coordinates before ancestor rotations; moving a frame/group translates all descendants. SVG src uses a base64 data:image/svg+xml URL. Changing id/kind is forbidden. A single undo step.", json!({"nodeId":string,"properties":node_properties()}), &["nodeId","properties"], false),
