...
pub fn catalog() -> Vec<Value> {
    ...
    let mut tools = vec![
        ...
        tool("create_artboard", "Create a page shell or section frame using world coordinates. Returns its node ID for later write_html calls.", json!({"name":string,"x":{"type":"number"},"y":{"type":"number"},"width":{"type":"number","minimum":40},"height":{"type":"number","minimum":40},"fill":string,"widthSizing":sizing_mode(),"heightSizing":sizing_mode(),"layout":layout_properties()}), &["name","width","height"], false),
