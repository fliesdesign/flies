...
Canvas spacing handles edit the same `layout.gap`, `layout.rowGap` and padding fields
as MCP. Setting side overrides enables independent side handles. Their live previews, commits and cancellation share document state, undo,
autosave and portable projects with property-panel and agent edits.

### Responsive layout, rich text and reusable artwork
...
Tools that target different files run in parallel. Calls that share a `fileId` (or a session's last opened file) stay FIFO while the editor is free. Up to 16 overlapping requests wait automatically. Pairing a write with a screenshot of the same file no longer produces busy retries; await the mutation when that screenshot must follow it. Opening a file for an agent adds a tab without changing the user's visible document. Queued calls time out after 45 seconds without being dispatched and are safe to retry. Idle polling waits rather than busy-looping. A dispatched call still times out after 45 seconds, but the editor keeps running and stores the result. Call `get_request` with the id from that timeout. It returns immediately and does not use the editor: `status: running`, or the original tool result once. While that request is unfinished, another tool for the same file also returns immediately with that id. After `get_request` returns the result, inspection can run. No open file, invalid input, or failed saves return tool errors.
