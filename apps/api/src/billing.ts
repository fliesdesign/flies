...
export const PLANS = {
  free: { designFiles: 5, imageUploadBytes: 30_000_000, mcpCallsPerWeek: 300, publicMcp: false, teamWorkspace: false, commercialUse: false, shareLinks: false },
  pro: { designFiles: 250, imageUploadBytes: 250_000_000, mcpCallsPerWeek: 500_000, publicMcp: true, teamWorkspace: true, commercialUse: true, shareLinks: true },
} as const;
export type Plan = keyof typeof PLANS;
export type Entitlements = { enabled: false; plan: null; limits: null } | { enabled: true; plan: Plan; limits: typeof PLANS[Plan] };
...
export type BillingState = {
  ...
  activeSubscriptions: { id: string; productId: string; status: string; currentPeriodEnd: Date; endsAt: Date | null }[];
};
...
export function polarProvider(config: NonNullable<ReturnType<typeof billingConfig>>, webUrl: string): BillingProvider {
  const polar = new Polar({ accessToken: config.accessToken, server: config.server });
  return {
    async state(workspaceId) {
      try { return await polar.customers.getStateExternal({ externalId: workspaceId }, { timeoutMs: 8000 }); }
      catch (error) { if (error instanceof ResourceNotFound) return null; throw error; }
    },
    async checkout(workspaceId, user, seats) {
      const checkout = await polar.checkouts.create({
        products: [config.productId], externalCustomerId: workspaceId,
        customerEmail: user.email, customerName: user.name, seats,
        successUrl: `${webUrl}/recents?billing=success`, returnUrl: `${webUrl}/recents`,
        metadata: { workspace_id: workspaceId },
      });
      return checkout.url;
    },
    async portal(workspaceId) {
      const session = await polar.customerSessions.create({ externalCustomerId: workspaceId, returnUrl: `${webUrl}/recents` });
...
export function proSubscription(state: BillingState | null, productId: string, now = new Date()) {
  return state?.activeSubscriptions.find((sub) => sub.productId === productId && sub.status === "active" && sub.currentPeriodEnd > now && (!sub.endsAt || sub.endsAt > now));
}
...
export function assertImageLimits(snapshot: Snapshot, entitlements: Entitlements) {
  ...
  for (const node of snapshot.nodes) {
    ...
    if (header.endsWith(";base64")) {
      ...
      bytes = payload.length * 3 / 4 - (payload.endsWith("==") ? 2 : payload.endsWith("=") ? 1 : 0);
    } else {
      try { bytes = new TextEncoder().encode(decodeURIComponent(payload)).byteLength; }
      catch { throw new HTTPException(400, { message: "Invalid image encoding." }); }
    }
    if (bytes > entitlements.limits.imageUploadBytes)
      throw new HTTPException(413, { message: `Your ${entitlements.plan} plan allows images up to ${entitlements.limits.imageUploadBytes / 1_000_000} MB.` });
  }
}
...
export function verifyPolarWebhook(body: string, headers: Record<string, string>, secret: string): unknown {
  for (const key of [secret, Buffer.from(secret).toString("base64")]) {
    try { return new Webhook(key).verify(body, headers); } catch { /* Try the legacy key. */ }
  }
...
export function billingService(db: Database, config: Config, injected?: BillingProvider) {
  ...
  const provider = settings ? injected ?? polarProvider(settings, config.WEB_URL.replace(/\/$/, "")) : null;
  ...
  async function entitlements(workspaceId: string, refresh = false): Promise<Entitlements> {
    ...
    return db.transaction(async (tx) => {
      ...
      let [cached] = await tx.select().from(workspaceBilling).where(eq(workspaceBilling.workspaceId, workspaceId));
      if (refresh || !cached || Date.now() - cached.checkedAt.getTime() >= 60_000) {
        const state = await provider.state(workspaceId);
        if (state && (state.organizationId !== settings.organizationId || state.externalId !== workspaceId))
          throw new HTTPException(502, { message: "Billing customer does not match this workspace." });
        const sub = proSubscription(state, settings.productId);
        const values = { workspaceId, customerId: state?.id ?? null, subscriptionId: sub?.id ?? null, proUntil: sub ? new Date(Math.min(sub.currentPeriodEnd.getTime(), sub.endsAt?.getTime() ?? Infinity)) : null, checkedAt: new Date() };
        [cached] = await tx.insert(workspaceBilling).values(values).onConflictDoUpdate({ target: workspaceBilling.workspaceId, set: values }).returning();
      }
  ...
  return {
    ...
    async checkout(workspaceId: string, user: Identity, seats: number) {
      ...
      if (current.plan === "pro") throw new HTTPException(409, { message: "Use the billing portal to manage your existing subscription." });
      return gateway.checkout(workspaceId, user, seats);
    },
    async portal(workspaceId: string) { return requireBilling().portal(workspaceId); },
    async webhook(body: string, headers: Record<string, string>) {
      requireBilling();
      const event = v.parse(v.object({ type: v.string(), data: v.unknown() }), verifyPolarWebhook(body, headers, settings!.webhookSecret));
      if (event.type !== "customer.state_changed") return;
      const state = v.parse(v.object({ organization_id: v.string(), external_id: v.nullable(v.string()) }), event.data);
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.update(workspaceBilling).set({ checkedAt: new Date(0) }).where(eq(workspaceBilling.workspaceId, workspaceId));
      });
    ...
    async consumeMcp(workspaceId: string, publicAccess: boolean) {
      ...
      if (publicAccess && !access.limits.publicMcp) throw new HTTPException(403, { message: "Public MCP access requires Pro." });
      const week = weekStart();
      const [usage] = await db.insert(mcpUsage).values({ workspaceId, week, calls: 1 }).onConflictDoUpdate({
        target: mcpUsage.workspaceId,
        set: { week, calls: sql`case when ${mcpUsage.week} = ${week} then ${mcpUsage.calls} + 1 else 1 end` },
      ...
      return { enabled: true, remaining: access.limits.mcpCallsPerWeek - usage.calls, resetsAt: new Date(week.getTime() + 7 * 86400_000).toISOString() };
    },
  };
}
